...
/**
 * @typedef {import("react-native-base").GridProps} Props
 * @param {Props} props
 */
const Grid = props => {
  const { style, children, numCols = 1, onPress, stretchVertical, rowStyle } = props;
  if (numCols === 1) {
    return <View style={style}>{children}</View>;
  }

  const rows = prepareData(children, numCols);
